...
- feat: publish an identifier→path index from the vault scanner, so a task identifier can later be resolved to its task file without walking the vault. `scanner.VaultScanner` gains `Resolve(ctx, id) (string, bool, error)`, and the scanner builds the inverse mapping from the `hashes` bookkeeping it already keeps, once per completed scan cycle, publishing it as a whole-map swap under a read-write lock so a reader sees either the previous cycle's map or the new one and never a partially built one. A duplicate identifier keeps both paths and is reported as `duplicate task_identifier %s in %s and %s` rather than silently collapsing to one; an identifier the index does not hold is reported absent with a nil error (a miss is not an error); and an entry whose identifier is empty — the halted-repair shape — is never indexed, so an empty identifier can never be resolved. The scanner's own bookkeeping stays single-goroutine and unguarded; only the published snapshot is shared. The consumer-side seam `result.TaskPathResolver` declares the same signature so the scanner satisfies it structurally without `pkg/scanner` importing `pkg/result` (spec 016)
- fix: resolve a task identifier from the scanner's published identifier→path index before walking the vault, so a result write for an identifier the index holds costs **one** file read and **zero** directory listings instead of one listing plus one read per file in the vault. `result.FindTaskFilePath` gains the resolver as its last parameter and consults it first; a hit reads exactly the one path the resolver observed and never builds a path from the identifier. The full walk stays as the fallback for an identifier the index does not hold, for a nil resolver (a test-only value), and for a hit whose single read or frontmatter parse fails — a stale entry lives at most one scan cycle, so the walk is the recovery. A resolver error is returned as-is and does **not** trigger a walk: an ambiguous identifier re-derived through the walk could silently resolve to one of the two files, the 2026-08-31 incident. `main.go` now constructs the scanner once and hands that one instance to both the sync loop and `factory.CreateCommandConsumer`, which threads it to the result writer and to all four executors that resolve a path directly (the planning-retry gate included); no production call site passes a nil resolver. The hit is logged at `glog.V(2)` as `index hit for task`, the deployed verbosity, so a running controller's behaviour is readable rather than inferred. The `notFoundAttempts` retry loop, its backoff, its logging, the `unowned`/`not_found` metric split, `commandExpireDuration`, the vault-routing guard's position before the lookup and the duplicate error's message shape are all unchanged (spec 016)
