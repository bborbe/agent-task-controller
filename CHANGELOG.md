...
- fix: resolve a task identifier from the scanner's published identifier→path index instead of walking the whole vault on every command, so a result write for an identifier the index holds costs **one** file read and **zero** directory listings instead of one listing plus one read per file — measured on the prod openclaw controller at 8,712 git-rest reads per lookup against an 8,972-file vault, ~11s per command, which serialized the single-threaded command consumer behind a full-vault read on every command and pushed tail commands past the 60-minute `commandExpireDuration` window where they were dropped as expired (one measured create-task was consumed 36 minutes late). The scanner already parsed every file's `task_identifier` to decide whether it changed and threw the mapping away; it now publishes it once per completed scan cycle — built whole from the `hashes` bookkeeping it already keeps and swapped under a read-write lock, so a reader sees either the previous cycle's map or the new one and never a partially built one — as `scanner.VaultScanner.Resolve(ctx, id) (string, bool, error)`. The consumer-side seam `result.TaskPathResolver` declares the same signature, so the scanner satisfies it structurally without `pkg/scanner` importing `pkg/result`; the scanner's own bookkeeping stays single-goroutine and unguarded and only the published snapshot is shared. `result.FindTaskFilePath` gains the resolver as its last parameter and consults it first: a hit reads exactly the one path the resolver observed, never builds a path from the identifier, and re-verifies that the file still carries the requested identifier before accepting it, so a path repurposed since the last cycle cannot receive another task's result. The full walk stays as the fallback for an identifier the index does not hold, for a nil resolver (a test-only value), and for a hit whose single read, frontmatter parse or identifier re-check fails — a stale entry lives at most one scan cycle, so the walk is the recovery. A resolver error is returned as-is and does **not** trigger a walk; an identifier the index does not hold is reported absent with a nil error (a miss is not an error); an entry whose identifier is empty — the halted-repair shape — is never indexed, so an empty identifier can never be resolved; and a duplicate identifier keeps both paths and is reported as `duplicate task_identifier %s in %s and %s` rather than silently collapsing to one. `main.go` now constructs the scanner once and hands that one instance to both the sync loop and `factory.CreateCommandConsumer`, which threads it to the result writer and to all four executors that resolve a path directly (the planning-retry gate included); no production call site passes a nil resolver. The hit is logged at `glog.V(2)` as `index hit for task`, the deployed verbosity, so a running controller's behaviour is readable rather than inferred, and the index adds no config field, env var, CLI flag or metric. The `notFoundAttempts` retry loop, its backoff, its logging, the `unowned`/`not_found` metric split, `commandExpireDuration`, the vault-routing guard's position before the lookup and the duplicate error's message shape are all unchanged (spec 016)
...
- feat: the result write-back merge gains a third ownership behaviour beside "keep the on-disk value" and "incoming wins" — **accumulate** — for the two cumulative counters `metrics_agent_turns` and `metrics_interaction_count`. When both the task file and the agent's result payload carry a numeric value for one of them, the written value is their sum, so a lifetime total is never reset to a single run's value (the silent reset the incoming-wins default would cause) and never discarded (what routing the keys through `controllerOwnedFields` would cause). An absent incoming key leaves the on-disk value exactly as it was, so an emitted `0` from an evidenced unattended run adds nothing; an absent on-disk key takes the emitted value as the starting total. Numbers are compared and added by numeric value (`int`/`int64`/`float64`), so a JSON-decoded incoming `float64` adds correctly to a YAML-decoded on-disk `int`. A non-numeric value on either side never panics and never overwrites: the on-disk value is kept verbatim and one guard decision names the field when the two values differ (none when they are equal). Accumulation is a transform, not a discard, so a clean sum produces no guard decision and no new log line; the frozen guard lists, the terminal-`status` pin and every other merge behaviour are unchanged (spec 012).
- test: lock the accumulated-counter sum invariant across two sequential `WriteResult` calls through the real result write-back path in `pkg/result/result_writer_test.go` — the first call's emitted `metrics_agent_turns: 3` lands on disk and the second call reads that file back and writes `7`, so the second run's own value is never the written total and the first run's is not lost, with `AtomicReadModifyWriteAndCommitPushCallCount` proving both write-backs were real; a second spec pins the terminal-`status` pin as not suppressing the accumulation, accumulating the incoming counter onto the on-disk total while the on-disk `status` stays pinned and the incoming one is discarded (spec 012)
- docs: record the accumulated-counter rule in `docs/controller-design.md` § "Frontmatter Merge" as a third ownership row beside controller-owned, operator-owned and agent-owned — `metrics_agent_turns` and `metrics_interaction_count` accumulate rather than being overwritten or discarded, with the sum rule, the absent-incoming and absent-on-disk rules, and the non-numeric keep-verbatim branch stated as shipped, plus the reason the keys belong on neither guard list: `controllerOwnedFields` discards the emitted value so the counter would never move, and the incoming-wins default would reset a lifetime total to one run's value, the silent reset the accumulation prevents (spec 012)

## v0.9.0

- feat: publish an `agent-escalation` notification into the shared notification core whenever a result write parks a task by clearing a non-empty assignee, so a parked task reaches Discord and Telegram without any per-consumer channel code. All four escalation rows are covered by one detection point — `applyTriggerCap`, `applyRetryCap`, `applyRetryCounter` and the spec-042 partial-update path all funnel through `applyRetryCounter` inside `buildResultModifyFn`'s modify closure — but the publish deliberately sits *outside* that closure, in the new `writeAndPublish`, because the closure re-runs on every git retry and a publish hooked into the `clearAssignee` chokepoint would fire once per attempt rather than once per escalation; the recorder is set-only for the same reason, so a retry that re-reads the now-empty on-disk assignee cannot erase an escalation an earlier attempt genuinely performed. The command sets `Type: agent-escalation` and no `Target`, leaving the channel decision to the deployed routing table, and carries the task identifier, task name and previous assignee in `Metadata` plus an Obsidian deeplink in the message. A publish failure never fails the write — the result is already committed, and losing the ping is better than rolling back a task write because a notification broker was briefly unavailable. Adds `github.com/bborbe/notification` v0.6.1 to `go.mod`, extends `NewResultWriter` with a `NotificationPublishCommandSender` (nil-safe, so callers that do not publish keep working), and wires the sender in `main.go` on the controller's own topic prefix.
