...
- [ ] The V(1) publish line is extended to carry the emitted deeplink, so the deployed encoder is observable in the pod log — evidence: the log call's argument list names the deeplink, verified by `grep -A8 -n 'assignee cleared → notification published' pkg/result/result_writer.go | grep -cE 'vaultDeeplink\(|deeplink'` returning ≥1. The check accepts an inline call or a variable assigned from one, because the spec requires the helper to stay the single owner of the URI contract — an assertion on the literal `obsidian://open` near the log statement would fail the correct implementation, and could only pass by duplicating the URI format string. **The behavioural proof that the line really carries a resolvable URI is Post-Deploy AC 8/9**, which read it from a running pod; this AC is a shape check only.
...
grep -A8 -n 'assignee cleared → notification published' pkg/result/result_writer.go | grep -cE 'vaultDeeplink\(|deeplink' || true
